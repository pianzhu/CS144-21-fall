...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    if (index > _capacity - _output.buffer_size() + _unassembled_p - 1) {
        return;
    }                  // the entire substring is not in slide window.
    ...
    if (eof && index + data.size() <=
                   _unassembled_p +
                       _output.remaining_capacity()) {  // mark that there is no data that will be written in bytestream
        _eof_end = true;
    }
...
size_t StreamReassembler::ack_index() const { return _unassembled_p; }